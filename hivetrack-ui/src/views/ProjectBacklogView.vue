...
onMounted(() => document.addEventListener('pointerdown', onEpicFilterClickOutside, true))
onBeforeUnmount(() => document.removeEventListener('pointerdown', onEpicFilterClickOutside, true))
        ...
        <div class="flex items-center gap-3">
          ...
          <div v-if="epics.length" ref="epicFilterRoot" class="relative">
            ...
            >
              ...
              <span class="text-sm" :class="selectedEpic ? 'text-slate-700 font-medium' : 'text-slate-500'">
                ...
                >
                  ...
                  <div class="max-h-52 overflow-y-auto py-1">
                    ...
                    <button
                      ...
                      :class="!selectedEpicId ? 'bg-slate-50 font-medium text-slate-900' : 'text-slate-500 hover:bg-slate-50'"
                      @click="selectEpicFilter(null)"
                    >
                      <CheckIcon v-if="!selectedEpicId" class="size-3.5 text-blue-500 flex-shrink-0" />
                    ...
                    <button
                      ...
                      :key="epic.id"
                      class="w-full flex items-center gap-2 px-3 py-1.5 text-sm text-left cursor-pointer transition-colors"
                      :class="epic.id === selectedEpicId ? 'bg-slate-50 font-medium text-slate-900' : 'text-slate-700 hover:bg-slate-50'"
                      @click="selectEpicFilter(epic.id)"
                    >
                      <CheckIcon v-if="epic.id === selectedEpicId" class="size-3.5 text-blue-500 flex-shrink-0" />
                      <LayersIcon v-else class="size-3.5 text-violet-400 flex-shrink-0" />
                      <span class="flex-1 min-w-0 truncate">{{ epic.title }}</span>
                    </button>

                    <p v-if="epicFilterSearch && !filteredEpicOptions.length" class="px-3 py-2 text-xs text-slate-400">
