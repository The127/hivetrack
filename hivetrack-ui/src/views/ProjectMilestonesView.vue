  ...
  <MainLayout>
    <div class="max-w-5xl mx-auto px-6 py-8">
      ...
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Milestones</h1>
        <div class="flex items-center gap-2">
          <!-- View toggle -->
          <div class="flex rounded-md border border-slate-200 dark:border-slate-700 overflow-hidden">
            <button
              class="px-2.5 py-1.5 text-xs flex items-center gap-1.5 transition-colors"
              :class="viewMode === 'list' ? 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-medium' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer'"
              @click="viewMode = 'list'"
            >
            ...
            <button
              class="px-2.5 py-1.5 text-xs flex items-center gap-1.5 border-l border-slate-200 dark:border-slate-700 transition-colors"
              :class="viewMode === 'timeline' ? 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-medium' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer'"
      ...
      <div v-else class="space-y-3">
        <div
          ...
          :class="m.closed_at ? 'border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900'"
        >
          <!-- Row: title + actions -->
          <div class="flex items-start justify-between gap-3">
            <div class="flex items-center gap-2 flex-wrap min-w-0">
              <span
                ...
                :class="m.closed_at ? 'text-slate-500 dark:text-slate-400 line-through' : 'text-slate-900 dark:text-slate-100'"
              >{{ m.title }}</span>
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
              <button
                class="p-1 rounded text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer"
              ...
              <button
                class="p-1 rounded text-slate-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors cursor-pointer"
          ...
          <!-- Description -->
          <p v-if="m.description" class="text-xs text-slate-500 dark:text-slate-400">{{ m.description }}</p>
      ...
      <div class="space-y-4">
        <div>
          ...
          <input
            ...
            class="w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            @keydown.enter="submitCreate"
          />
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Description <span class="font-normal text-slate-400 dark:text-slate-500">(optional)</span></label>
          <textarea
            v-model="formDescription"
            rows="3"
            placeholder="What does this milestone represent?"
            class="w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
          />
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Target date <span class="font-normal text-slate-400 dark:text-slate-500">(optional)</span></label>
          <input
            v-model="formTargetDate"
            type="date"
            class="w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          />
        </div>
      </div>
      <template #footer>
