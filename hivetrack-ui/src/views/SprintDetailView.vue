...
import {
  ...
  statusLabel,
  statusScheme,
} from "@/composables/issueConstants";
...
const sprint = computed(
  () =>
    (sprintsResult.value?.sprints ?? []).find((s) => s.id === sprintId.value) ??
    null,
);
...
const { data: issuesResult, isLoading: issuesLoading } = useQuery({
  ...
  queryFn: () =>
    fetchIssues(slug.value, { sprint_id: sprintId.value, limit: 500 }),
  ...
  <MainLayout>
    <div class="max-w-3xl mx-auto px-6 py-8">
      ...
      <template v-else-if="sprint">
        ...
        <div class="mb-6 space-y-1">
          <div class="flex items-center gap-3">
            ...
            <span v-if="dateRange(sprint)" class="text-sm text-slate-400">{{
              dateRange(sprint)
            }}</span>
          </div>
          <p
          ...
          <div class="flex items-center gap-3 pt-1">
            ...
            <span class="text-xs text-slate-400"
              >{{ sprint.done_count }} / {{ sprint.issue_count }} issues
              done</span
            >
          </div>
        </div>
        ...
        <div v-if="groupedIssues.length === 0">
          <EmptyState
            title="No issues"
            description="This sprint had no issues."
          />
        </div>
        ...
        <div v-else class="space-y-5">
          <div v-for="group in groupedIssues" :key="group.status">
            <div class="flex items-center gap-2 mb-2">
              <Badge :color-scheme="statusScheme(group.status)" compact>{{
            ...
            >
              ...
              >
                <span
                  class="text-[11px] font-mono text-slate-400 flex-shrink-0 w-20"
                >
      ...
      <EmptyState
        ...
        description="This sprint does not exist or has been deleted."
      />
    </div>
  </MainLayout>
</template>
