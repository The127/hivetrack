...
<template>
  ...
  >
    <div class="flex flex-col gap-4">
      <div class="space-y-2">
        ...
        >
          <input
            ...
            class="flex-1 rounded-md border border-slate-200 px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
            @keydown.enter.prevent="submit"
          />
          <button
            v-if="titles.length > 2"
            type="button"
            class="flex-shrink-0 rounded-md p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors cursor-pointer"
      ...
      <button
        type="button"
        class="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 transition-colors cursor-pointer w-fit"
        @click="addTitle"
      >
