<!--
  AssigneeSelect — multi-select dropdown for picking issue assignees.

  Props:
    projectSlug — project slug (to fetch members from project query)
    modelValue  — []{ id, display_name, avatar_url } (current assignees)

  Emits:
    update:modelValue — []uuid (selected assignee IDs)
-->
<script setup>
import { ref, computed, nextTick, onMounted, onBeforeUnmount } from 'vue'
import { useQuery } from '@tanstack/vue-query'
import { fetchProject } from '@/api/projects'
import { UsersIcon, SearchIcon, CheckIcon } from 'lucide-vue-next'
    ...
    >
      <template v-if="modelValue.length">
        <div class="flex -space-x-1">
          <Avatar
            v-for="a in modelValue.slice(0, 3)"
            :key="a.id"
            :name="a.display_name"
            :src="a.avatar_url"
            size="xs"
      ...
      <template v-else>
        <span class="flex-1 text-slate-400">Unassigned</span>
      </template>
    </button>
