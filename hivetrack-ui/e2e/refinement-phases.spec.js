...
const SESSION_2 = '**/api/v1/projects/e2e-test/issues/2/refinement/session'
const SESSION_3 = '**/api/v1/projects/e2e-test/issues/3/refinement/session'
...
test.describe('Refinement Phase UI', () => {
  test('refinement panel opens and shows start button when no session', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /start refinement/i })).toBeVisible({ timeout: 5_000 })
  })

  test('phase stepper shows all five phases when refinement panel is opened', async ({ page }) => {
    ...
    await expect(page.getByText('Actor & Goal').first()).toBeVisible({ timeout: 10_000 })
  ...
  test('story progress shows BDD scenarios when bdd_scenarios phase_result is present', async ({ page }) => {
      ...
      messages: [
        {
          id: 'msg-proposal',
          role: 'assistant',
          content: '{"type":"proposal","title":"Password Reset","description":"## User Story"}',
    ...
    await expect(page.getByText('BDD Scenarios').first()).toBeVisible({ timeout: 10_000 })

    // The scenario name and steps should be visible in Story Progress
    await expect(page.getByText('Successful password reset')).toBeVisible({ timeout: 5_000 })
    await expect(page.getByText('a registered user with email user@example.com')).toBeVisible()
