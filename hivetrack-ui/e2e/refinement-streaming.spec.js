...
test.describe("Refinement streaming UI", () => {
  test("shows spinner when generating but no tokens received yet", async ({
    page,
  }) => {
    ...
    await expect(
      page
        .locator('.animate-spin, [data-testid="spinner"], svg.animate-spin')
        .first(),
    ).toBeVisible({ timeout: 10_000 });
  });

  test("shows streaming bubble with partial response while generating", async ({
    page,
  }) => {
  ...
  test("shows question card when agent has responded and is not generating", async ({
    page,
  }) => {
  ...
  test("does not show streaming bubble when generation stops", async ({
    page,
  }) => {
