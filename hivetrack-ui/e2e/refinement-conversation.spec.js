...
const ACTOR_QUESTION =
  "Who is the primary actor that benefits from this feature?";
...
test.describe("Refinement conversation flow", () => {
  test("thinking spinner resolves to first question", async ({ page }) => {
    ...
    await page.route(SESSION_URL, (route) => {
      ...
      if (callCount <= 2) {
        route.fulfill({
          json: { ...BASE_SESSION, is_generating: true, partial_response: "" },
        });
      } else {
        route.fulfill({
          json: {
    ...
    await expect(page.getByText("Hivemind is thinking...")).toBeVisible({
      timeout: 10_000,
    });

    // Spinner resolves to first question (polling picks up the completed response)
    await expect(page.getByText(ACTOR_QUESTION, { exact: false })).toBeVisible({
      timeout: 15_000,
    });
    await expect(page.getByText("Hivemind is thinking...")).not.toBeVisible();
  });

  test("sending a reply triggers message POST and shows next question", async ({
    page,
  }) => {
    ...
    await expect(page.getByText(userReply, { exact: false })).toBeVisible({
      timeout: 10_000,
    });
    await expect(page.getByText(GOAL_QUESTION, { exact: false })).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test("suggestion chips send pre-filled reply on click", async ({ page }) => {
    ...
    await expect
      .poll(() => messageBody?.content, { timeout: 5_000 })
      .toBe("End user");
  });
});
